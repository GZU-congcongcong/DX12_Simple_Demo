...
void SSAOApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->SetGraphicsRootShaderResourceView(2, matBufferAddr);
	mCommandList->SetGraphicsRootDescriptorTable(4, GetGpuSrv(0));
	...
	// 这里只需要清空渲染目标即可, 因为深度值在绘制深度/法线纹理时已经获取, 且不需要改变.
	...
	mCommandList->SetGraphicsRootDescriptorTable(3, GetGpuSrv(mSkyTexHeapIndex));
	mCommandList->SetGraphicsRootDescriptorTable(4, GetGpuSrv(0));
	...
	DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::ShadowDebug]);*/
...
void SSAOApp::UpdateMainPassCB(const GameTimer& gt)
{
	...
	mMainPassCB.AmbientLight = { 0.3f, 0.3f, 0.3f, 0.0f };
	mMainPassCB.Lights[0].Direction = mRotatedLightDirections[0];
	mMainPassCB.Lights[0].Strength = { 120.4f, 120.4f, 120.5f };
...
void SSAOApp::BuildRootSignature()
{
	...
	texTable1.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 6, 4, 0);
...
void SSAOApp::BuildSSAORootSignature()
{
	...
	slotRootParameter[1].InitAsConstants(1, 1, 0);				// Blur Dir.
...
void SSAOApp::BuildDescriptorHeaps()
{
	// Basic : Tex(7)
	// Shadow : Shadow(1)
	// SSAO : Normal(1) + Depth(1) + RandomVector(1) + SSAO(2)
	...
	/*mShadowDebugIndex = mShadowMapHeapIndex + 1;
	auto nullCpuSrv = GetCpuSrv(mShadowDebugIndex);
	srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
	...
	mNullSrv = GetGpuSrv(mShadowDebugIndex);*/
...
void SSAOApp::BuildShadersAndInputLayout()
{
	...
	mShaders["ShadowdebugVS"] = d3dUtil::CompileShader(L"Shaders/ShadowDebug.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["ShadowdebugPS"] = d3dUtil::CompileShader(L"Shaders/ShadowDebug.hlsl", nullptr, "PS", "ps_5_1");
...
void SSAOApp::BuildShapesGeometry()
{
	...
	GeometryGenerator::MeshData SSAOquad = geoGen.CreateQuad(0.0f, 0.0f, 1.0f, 1.0f, 0.0f);
	//GeometryGenerator::MeshData Shadowquad = geoGen.CreateQuad(5.0f, 0.0f, 1.0f, 1.0f, 0.0f);
	...
	UINT SSAOquadVertexOffset = (UINT)cylinder.Vertices.size() + cylinderVertexOffset;
	//UINT ShadowquadVertexOffset = (UINT)SSAOquad.Vertices.size() + SSAOquadVertexOffset;
	...
	UINT SSAOquadIndexOffset = (UINT)cylinder.Indices32.size() + cylinderIndexOffset;
	//UINT ShadowquadIndexOffset = (UINT)SSAOquad.Indices32.size() + SSAOquadIndexOffset;
	...
	SubmeshGeometry SSAOquadSubmesh;
	SSAOquadSubmesh.IndexCount = (UINT)SSAOquad.Indices32.size();
	SSAOquadSubmesh.StartIndexLocation = SSAOquadIndexOffset;
	SSAOquadSubmesh.BaseVertexLocation = SSAOquadVertexOffset;

	/*SubmeshGeometry ShadowquadSubmesh;
	ShadowquadSubmesh.IndexCount = (UINT)Shadowquad.Indices32.size();
	ShadowquadSubmesh.StartIndexLocation = ShadowquadIndexOffset;
	ShadowquadSubmesh.BaseVertexLocation = ShadowquadVertexOffset; */

	///
	/// 顶点缓冲区合并.
	///
	std::vector<Vertex> vertices(box.Vertices.size() + grid.Vertices.size() + sphere.Vertices.size() +
		cylinder.Vertices.size() + SSAOquad.Vertices.size());// +Shadowquad.Vertices.size());// );
	UINT k = 0;
	...
	for (UINT i = 0; i < (UINT)SSAOquad.Vertices.size(); ++i, ++k)
	{
		vertices[k].Pos = SSAOquad.Vertices[i].Position;
		vertices[k].Normal = SSAOquad.Vertices[i].Normal;
		vertices[k].TexC = SSAOquad.Vertices[i].TexC;
		vertices[k].Tangent = SSAOquad.Vertices[i].TangentU;
	}

	/*for (UINT i = 0; i < (UINT)Shadowquad.Vertices.size(); ++i, ++k)
	{
		vertices[k].Pos = Shadowquad.Vertices[i].Position;
		vertices[k].Normal = Shadowquad.Vertices[i].Normal;
		vertices[k].TexC = Shadowquad.Vertices[i].TexC;
		vertices[k].Tangent = Shadowquad.Vertices[i].TangentU;
	}*/
	...
	indices.insert(indices.end(), SSAOquad.GetIndices16().begin(), SSAOquad.GetIndices16().end());
	//indices.insert(indices.end(), Shadowquad.GetIndices16().begin(), Shadowquad.GetIndices16().end());
	...
	geo->DrawArgs["SSAOquad"] = SSAOquadSubmesh;
	//geo->DrawArgs["Shadowquad"] = ShadowquadSubmesh;
...
void SSAOApp::BuildSkullGeometry()
{
	...
	for (UINT i = 0; i < vcount; ++i)
	{
		...
		// 求出纹理坐标.
		float theta = atan2f(vertices[i].Pos.z, vertices[i].Pos.x);
		float phi = acosf(vertices[i].Pos.y);

		if (theta < 0) theta += XM_2PI;
...
void SSAOApp::BuildRenderItems()
{
	...
	auto SSAOquadRitem = std::make_unique<RenderItem>();
	SSAOquadRitem->World = MathHelper::Identity4x4();
	SSAOquadRitem->TexTransform = MathHelper::Identity4x4();
	SSAOquadRitem->ObjectCBIndex = 1;
	SSAOquadRitem->Mat = mMaterials["bricks"].get();
	SSAOquadRitem->Geo = mGeometries["shapeGeo"].get();
	SSAOquadRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	SSAOquadRitem->IndexCount = SSAOquadRitem->Geo->DrawArgs["SSAOquad"].IndexCount;
	SSAOquadRitem->StartIndexLocation = SSAOquadRitem->Geo->DrawArgs["SSAOquad"].StartIndexLocation;
	SSAOquadRitem->BaseVertexLocation = SSAOquadRitem->Geo->DrawArgs["SSAOquad"].BaseVertexLocation;
	mRitemLayer[(int)RenderLayer::SSAODebug].push_back(SSAOquadRitem.get());
	mAllRitems.push_back(std::move(SSAOquadRitem));

	//auto ShadowquadRitem = std::make_unique<RenderItem>();
	//ShadowquadRitem->Instances.resize(1);
	//XMStoreFloat4x4(&ShadowquadRitem->World, XMMatrixTranslation(0, -2.0f, 0));
	//ShadowquadRitem->TexTransform = MathHelper::Identity4x4();
	//XMStoreFloat4x4(&ShadowquadRitem->Instances[0].World, XMMatrixTranslation(0, -2.0f, 0));
	//ShadowquadRitem->Instances[0].TexTransform = MathHelper::Identity4x4();
	//ShadowquadRitem->ObjectCBIndex = 2;
	//ShadowquadRitem->Mat = mMaterials["bricks"].get();
	//ShadowquadRitem->Geo = mGeometries["shapeGeo"].get();
	//ShadowquadRitem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	//ShadowquadRitem->IndexCount = ShadowquadRitem->Geo->DrawArgs["Shadowquad"].IndexCount;
	//ShadowquadRitem->StartIndexLocation = ShadowquadRitem->Geo->DrawArgs["Shadowquad"].StartIndexLocation;
	//ShadowquadRitem->BaseVertexLocation = ShadowquadRitem->Geo->DrawArgs["Shadowquad"].BaseVertexLocation;
	//mRitemLayer[(int)RenderLayer::ShadowDebug].push_back(ShadowquadRitem.get());
	//mAllRitems.push_back(std::move(ShadowquadRitem));
...
void SSAOApp::BuildPSOs()
{
	...
	///
	/// 不透明物体PSO, 因为需要绘制深度/法线纹理, 在这里就不需要进行深度写入, 但还是需要深度测试.
	///
	...
	/// 这里显示ShadowDebug纹理.
	///
	D3D12_GRAPHICS_PIPELINE_STATE_DESC ShadowdebugPsoDesc = basePsoDesc;
	...
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&ShadowdebugPsoDesc,
		IID_PPV_ARGS(mPSOs["Shadowdebug"].GetAddressOf())));
	...
	///
	/// SSAO绘制的PSO. SSAO不需要深度写入, 也不需要深度测试, 深度相关信息已经在此前获取.
	///
	...
	///
	/// SSAO因为降采样会有很多噪点, 因此需要一个模糊过程的PSO.
	///
	...
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&ssaoBlurPsoDesc,
		IID_PPV_ARGS(mPSOs["ssaoBlur"].GetAddressOf())));

	///
	/// 调试所用的PSO, 在这里显示SSAO纹理.
	///
	D3D12_GRAPHICS_PIPELINE_STATE_DESC SSAOdebugPsoDesc = basePsoDesc;
	SSAOdebugPsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(mShaders["SSAOdebugVS"]->GetBufferPointer()),
		mShaders["SSAOdebugVS"]->GetBufferSize()
	};
	SSAOdebugPsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(mShaders["SSAOdebugPS"]->GetBufferPointer()),
		mShaders["SSAOdebugPS"]->GetBufferSize()
	};
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&SSAOdebugPsoDesc,
		IID_PPV_ARGS(mPSOs["SSAOdebug"].GetAddressOf())));
...
void SSAOApp::DrawSceneToShadowMap()
{
	...
	// 默认先绘制阴影贴图, 所以在重置PSO时已经设置成了阴影的PSO.
...
void SSAOApp::DrawNormalAndDepthMap()
{
	mCommandList->RSSetViewports(1, &mScreenViewport);
	mCommandList->RSSetScissorRects(1, &mScissorRect);
